...
async function hashPassword(password,salt=randomHex(16),iterations=100000){
...
if (url.pathname === "/api/auth/setup" && req.method === "POST") {
  try {
    console.log("Administrator setup started");
    ...
    if (!env.ADMIN_SETUP_TOKEN) {
      console.error("ADMIN_SETUP_TOKEN is missing");
    ...
    if (
      String(b.token || "").trim() !==
      String(env.ADMIN_SETUP_TOKEN).trim()
    ) {
    ...
    if (!isSuper(e)) {
      return json(
        { error: "Use an approved administrator email." },
        403
      );
    }
    ...
    await env.DB
      ...
      .bind(
        crypto.randomUUID(),
        e,
        String(b.name || "Administrator").trim(),
        "admin",
        h.hash,
        h.salt,
        h.iterations
